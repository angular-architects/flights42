...
**Re-executed 2026-07-29.** The 2026-07-28 session's changes were not
persisted — the branch still carried the pre-migration state (0.2.0, no
changelog, none of the deletions). The migration was re-run in full, using
this log as the verified blueprint; every gate reproduced the documented
results (same single pre-existing a2ui network failure, no new failures;
dev server boots and serves; live proxied `tools/call` by serverId and
`resources/read` by real serverHash against `mastra dev` + the running MCP
server). Deviations from the first run: step 4 was implemented directly in
its final hybrid form (no full-middleware detour and revert), and the
step-2 gate additionally verified the `AppHttpAgent.addMessage` guard
negatively (guard disabled → the synthetic tool message shadows the server
result and the roundtrip spec fails). The changelog lives at
