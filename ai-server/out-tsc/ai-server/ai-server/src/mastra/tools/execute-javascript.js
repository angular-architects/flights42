 ...
 */
function marshalFlights(context, flights) {
...
function setStringProp(context, target, key, value) {
  const handle = context.newString(value);
  context.setProp(target, key, handle);
  handle.dispose();
}
function setNumberProp(context, target, key, value) {
 ...
 */
async function runInSandboxInner(code) {
