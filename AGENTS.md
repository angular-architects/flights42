...
- Use the local Angular CLI via `npx ng ...` from the repository root.
- After Angular code or configuration changes, run `npx ng build`.
- `ng serve` is intentionally configured in `angular.json` with
  `host: 0.0.0.0` and `allowedHosts: true` so the dev server is reachable from
  the sandbox environment.
- Do not use the old `--disable-host-check` flag. Angular 21 no longer accepts
  it; use `--allowed-hosts` if a command-line override is needed.
- The production `anyComponentStyle` warning budget is currently `10kB`; this
  avoids warning on the dashboard stylesheet while keeping the `16kB` error
  limit in place.
